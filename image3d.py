...
class ImageDataGenerator3D(object):
    ...
    yklee: This version takes an image/mask as a target
	...
	#print("Info: Performing batch augmentation")
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                # yklee apply same to mask
                img = array_to_img(bY[i], self.dim_ordering, scale=True)
                fname = '{prefix}_{index}.mask.{format}'.format(prefix=self.save_prefix,
                                                           index=current_index + i,
                                                           format=self.save_format)
                img.save(os.path.join(self.save_to_dir, fname))
    ...
    # yklee: modified to support 3d output
    def random_transform(self, x, y):
        ...
        # yklee: apply transforms to y
        y = apply_transform(y, transform_matrix, img_channel_index,
                            fill_mode="constant", cval=self.cval)

        # yklee: add blurring 
        if self.blur_range != 0.:
            x = ndimage.gaussian_filter(x, sigma=np.random.uniform(0.,self.blur_range))
        
        if self.channel_shift_range != 0:
            x = random_channel_shift(x, self.channel_shift_range, img_channel_index)
            # yklee: don't apply channel shifts to mask
            
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                x = flip_axis(x, img_col_index)
                # yklee: apply flips to mask 
    ...
    def fit(self, X,
        ...
        if augment:
            ...
            aY = np.zeros(tuple([rounds * X.shape[0]] + list(X.shape)[1:]))
            for r in range(rounds):
                for i in range(X.shape[0]):
                    [aX[i + r * X.shape[0]], aY[i + r * X.shape[0]]] = self.random_transform(X[i],X[i])
            X = aX
