...
def random_gamma(image, gamma=1.):
...
class ImageDataGeneratorMultiOutput(object):
	...
	# yklee - check only one of the list
	...
	# yklee - instead of treating y as a direct output, append it to list
	bY = list()
	for yl in self.y:
		bY.append(yl[index_array])
	...
	#    x = random_gamma(x, gamma=1.+np.random.uniform(-self.gamma_range,self.gamma_range))
